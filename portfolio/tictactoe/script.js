    ...
    renderBoard: function () {
        ...
        for (var i = 0; i < this.board.length; i++) {
            ...
            for (var j = 0; j < this.board.length; j++) {
                $(".board-row-" + i).append("<td class='board-space' data-row='" + i + "' data-col='" + j + "'>" + this.board[i][j] + "</td>")
            }
        }
    },
...
var game = {
    ...
    addTurnText: function () {
        $(".board-container").append("<div class='turn-text'>" + this.currentPlayer + " Turn</div>");
    },
    takeTurn: function () {
        $(".board-space").on("click", function () {
            if (!game.gameover && game.validMove($(this))) {
                ...
                if (game.win(game.currentPlayer)) {
                    ...
                    $(".turn-text").text(game.currentPlayer + " Turn");
                }
            }
        });
    },
    ...
    winScreen: function (symbol) {
        this.gameover = true;
        $(".board-container").empty();
        $(".board-container").append("<p class='turn-text'> Game Over </p>");
        $(".board-container").append("<h1 class='result'>" + symbol + "</br> WINNER!</h1>");
        game.currentPlayer = "X";
        $(".result").on("click", game.init.bind(game));
    ...
    tieScreen: function () {
        ...
        $(".board-container").empty();
        $(".board-container").append("<p class='turn-text'> Game Over </p>");
        $(".board-container").append("<h1 class='result'>XO </br> DRAW!</h1>");
        game.currentPlayer = "X";
        $(".result").on("click", game.init.bind(game));
        
    },
    restart: function () {
        $(".restart-btn").on("click", game.init.bind(game));
    },
}